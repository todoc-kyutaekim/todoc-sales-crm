...
search.get('/', async (c) => {
  ...
  const [hospR, docR, meetR, paperR] = await Promise.all([
    c.env.DB.prepare('SELECT id, name, region, grade, status FROM hospitals WHERE name LIKE ? OR region LIKE ? OR address LIKE ? LIMIT 10').bind(s, s, s).all(),
    ...
    c.env.DB.prepare('SELECT m.id, m.meeting_date, m.meeting_type, m.purpose, d.name as doctor_name, h.name as hospital_name, m.hospital_id FROM meetings m LEFT JOIN doctors d ON m.doctor_id=d.id LEFT JOIN hospitals h ON m.hospital_id=h.id WHERE d.name LIKE ? OR h.name LIKE ? OR m.purpose LIKE ? OR m.content LIKE ? ORDER BY m.meeting_date DESC LIMIT 10').bind(s, s, s, s).all(),
