...
search.get('/', async (c) => {
  ...
  if (!q || q.trim().length < 1) return c.json({ data: { hospitals: [], doctors: [], meetings: [], papers: [], clinics: [] } })

  const s = `%${safeLike(q.trim())}%`

  const [hospR, docR, meetR, paperR, clinicR] = await Promise.all([
    ...
    c.env.DB.prepare(`SELECT cl.id, cl.name, cl.region, cl.priority, cl.status FROM clinics cl WHERE cl.name LIKE ? OR cl.region LIKE ? OR cl.address LIKE ? LIMIT 10`).bind(s, s, s).all(),
  ])
