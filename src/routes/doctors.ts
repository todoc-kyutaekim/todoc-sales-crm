...
doctors.post('/doctors', async (c) => {
  ...
  const r = await c.env.DB.prepare('INSERT INTO doctors (hospital_id,name,department,position,phone,email,specialty,influence_level,notes,photo,bio,education,career,clinic_hours,profile_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
    .bind(b.hospital_id, b.name.trim(), b.department || '', b.position || '', b.phone || '', b.email || '', b.specialty || '', b.influence_level || 'medium', b.notes || '', b.photo || '', b.bio || '', b.education || '', b.career || '', b.clinic_hours || '', b.profile_url || '').run()
...
doctors.put('/doctors/:id', async (c) => {
  ...
  if (!b.name || typeof b.name !== 'string' || b.name.trim().length === 0) return c.json({ error: 'name is required' }, 400)
  await c.env.DB.prepare('UPDATE doctors SET hospital_id=?,name=?,department=?,position=?,phone=?,email=?,specialty=?,influence_level=?,notes=?,photo=?,bio=?,education=?,career=?,clinic_hours=?,profile_url=?,updated_at=CURRENT_TIMESTAMP WHERE id=?')
    .bind(b.hospital_id, b.name.trim(), b.department || '', b.position || '', b.phone || '', b.email || '', b.specialty || '', b.influence_level || 'medium', b.notes || '', b.photo || '', b.bio || '', b.education || '', b.career || '', b.clinic_hours || '', b.profile_url || '', id).run()
...
doctors.patch('/doctors/:id/profile', async (c) => {
  ...
  if (b.profile_url !== undefined) { sets.push('profile_url=?'); vals.push(b.profile_url) }
