...
dashboard.get('/', async (c) => {
  const [hospitals, doctors, meetings, monthMeetings, lastMonthMeetings, recentMeetings, upcomingActions, regionStats, ciLatest, monthlyTrend, reminders] = await Promise.all([
    ...
    c.env.DB.prepare(`
      SELECT m.*, d.name as doctor_name, d.photo as doctor_photo, h.name as hospital_name
      FROM meetings m LEFT JOIN doctors d ON m.doctor_id=d.id LEFT JOIN hospitals h ON m.hospital_id=h.id
