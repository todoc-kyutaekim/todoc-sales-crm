...
- 글로벌 검색 (기관, 의료진, 미팅, 논문)
- 인라인 스타일 제거 → CSS 클래스 기반 반응형 필터

#### 모바일 UX (PWA)
- **Pull-to-Refresh** (터치 당기기로 새로고침)
- **오프라인 모드** (Service Worker API 응답 캐싱)
- **리마인더 알림 배지** (하단 네비게이션)
- 인라인 min-width/max-width 스타일 제거 (CSS 클래스 대체)
- 반응형 필터 레이아웃, 터치 최적화
...
| doctors | name, department, position, specialty, influence_level, photo, bio, education, career |
...
4. **미팅 기록** 추가 시 미팅 템플릿 선택, 복수 교수 선택 가능
5. **캘린더 뷰**에서 일정 확인, 미팅 통계에서 빈도/패턴 분석
6. ⭐ **즐겨찾기**로 자주 방문하는 기관/의료진 빠른 접근
7. 🏷 **태그**로 기관/의료진 분류 (CI 관심, 핵심 거래처 등)
8. **인공와우 통계** 메뉴에서 시장 데이터 확인
9. **Excel/CSV 내보내기**로 데이터 다운로드
