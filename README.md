...
- **보정 경유지가 있어도 업체명·지역이 어긋나지 않음**: 카카오 경로에 보정 경유지를
  끼워 넣으면 구간 수가 방문지 수보다 많아져, 예전에는 양식에서 업체명과 지역이
  한 칸씩 밀리거나 업체명 칸에 경유지 이름이 실렸습니다.
  - 지점마다 방문지 표식을 달아 되돌려 받으므로 순번 계산을 하지 않습니다
  - 경유지에서 끊긴 구간은 앞 방문지까지 이어 붙여 **총 주행거리는 그대로 보존**됩니다
  - 경유지 자체를 확인해야 할 때는 「산출근거 구간별」 시트에 카카오 원본 구간이 그대로 남습니다
  - ⚠️ 이 수정으로 경로 캐시 키가 바뀌어, 지난 달 재출력 시 카카오를 한 번 다시 조회합니다
...
- **보고서 다운로드**: CSV(BOM, 4섹션) / **XLSX**(사내 실제 정산 양식 재현, 재무팀 제출용)
  - 담당자마다 3시트: `〈이름〉`(월 교통비 정산내역) · `〈이름〉 증빙`(영수증 첨부용) ·
    `〈이름〉 증빙 톨게이트`(하이패스 내역 붙여넣기용 21열)
  - 뒤에 산출근거 4시트: 요약 / 일자별 운행기록 / 구간별 이동상세 / 방문기관 좌표
  - **편도 구간 1건 = 1행** 구조. 유류비·합계·월 청구액은 값이 아니라 **엑셀 수식**으로 들어가
    재무팀이 그 자리에서 검산·수정할 수 있습니다
    (유류비 `=기준유가 ÷ 연비 × 주행거리`, 차종에 따라 전기차/내연기관 블록을 자동 참조)
  - 차량정보 블록(연비·기준유가)은 담당자 설정값으로 채우고, 비어 있으면 `직접 입력` + ⚠️ 경고를 넣습니다
    (기준유가 출처: 휘발유/경유/LPG는 OPINET, 전기는 무공해차 통합누리집)
  - **월 감가상각비 400,000원 고정** (`MONTHLY_DEPRECIATION`)
...
#### 인공와우 통계
- S5800 인공와우이식술 **2010~2025년 16개년** 통계 시각화
- 건강보험심사평가원 보건의료빅데이터 기반 (**실제 데이터, 추정치 없음**)
- 연도별/성별/연령대별/지역별/기관종별 분석
- **데이터 커버리지**: 총 1,424행 (연 89행 × 16개년)
  - `ci_inpatient_outpatient` 112행 · `ci_age10_stats` 336행 · `ci_age5_stats` 592행
  - `ci_region_stats` 288행 · `ci_institution_stats` 96행
  - 마이그레이션: `migrations/0055_ci_stats_2010_2025.sql`
    (`INSERT OR REPLACE` + `UNIQUE(year, ...)` → **재실행 안전 / 멱등**)
  - HIRA 원본에서 `-`(해당 연도 실적 없음)로 표기된 칸은 **0으로 저장**합니다
  - 단위: 환자수(명) / 총사용량(회) / 진료금액(**천원**)
- **연도 하드코딩 없음** — 인사이트 문구(`N년간 총 진료금액`, `2010-2025 누적`)는
  DB에 실제로 존재하는 최소/최대 연도에서 계산합니다. 새 연도를 넣으면 코드 수정 없이 반영됩니다.
  (서버 `cistats.ts` 와 화면 `app.js` **양쪽 모두** 동적 계산으로 통일)

##### 심층 분석 탭 (`심층 분석`)
16개년 데이터에서 계산한 구조 분석 + 전망입니다.

> **🔴 실측과 추정의 분리 원칙**
> 이 탭은 두 구역으로 나뉘며, 화면에서도 되돌릴 수 없게 시각적으로 분리됩니다.
> - **실측 구역** — 실선 테두리 + 🟢`실측` 배지. HIRA 원본에서 직접 집계한 값만.
> - **추정 구역** — **점선 테두리** + 🟠`추정 · 실측 아님` / 🔴`추정 · 가정 기반` 배지.
>   API 응답에서도 `analytics`(실측)와 `forecast`·`deviceMarket`(추정)으로 키가 분리되어 있고,
>   두 추정 블록에는 `isEstimate: true` 플래그와 `caveat` 문구가 함께 담깁니다.
> 실사·투자 자료에 인용할 때는 반드시 어느 구역의 값인지 함께 표기하세요.

- **연령 코호트 역전** (`analytics.cohortTrend`)
  - 소아(0-19세) 비중 **56.1% → 18.7%** (-37.4%p), 60세 이상 **14.5% → 51.1%** (+36.6%p)
  - 60세 이상 환자수 79명 → 610명 (**7.7배**, CAGR +14.6%/년)
  - 소아 환자수는 305명 → 223명으로 **절대수까지 감소** (CAGR -2.1%/년)
  - → 이 시장은 「선천성 난청 소아」에서 「노인성 난청」으로 수요 축이 이동했습니다
- **지역 집중도** (`analytics.concentration`) — 서울/수도권 점유율 + HHI(허핀달지수)
  - 서울 단독 62.9% → 56.5%로 완화되었으나, 수도권 합계는 **76.8% → 80.5%로 오히려 심화**
  - 경기 CAGR +9.9% (68 → 281명) — 서울 수요가 경기로 이동
- **진료금액 증가 요인 분해** (`analytics.amountDecomp`)
  - 연도별 금액 증가율을 **물량(수술건수) 기여**와 **단가(건당금액) 기여**로 분리
  - 16년 누적: 금액 **+511%** = 물량 +139% × 단가 +155% → **단가 상승이 절반 이상 기여**
  - 건당 진료금액 57.8만원 → 147.7만원
- **지역별 성장률** (`analytics.regionGrowth`) — 지역별 16개년 환자수 CAGR·누적
  - 상위: 충남 +16.6% / 강원 +10.1% / 경기 +9.9% / 광주 +9.6%
  - 감소: 부산 -2.8% / 인천 -3.1% / 울산 -4.5%
  - ⚠️ 시작 또는 최신 연도 환자수가 0명인 지역(경북·전남·제주·충북·세종)은 CAGR을
    수학적으로 정의할 수 없어 `null` → 화면에 **「산출 불가」**로 표시합니다 (0이나 Infinity로 왜곡하지 않음)

**🟢 실측 구역 — 확장 지표 (2026-09-03 추가)**

- **성장 국면 전환점 탐지** (`analytics.breakScan` / `breakYear` / `breakRule`)
  - 시작연도를 한 해씩 뒤로 옮기며 선형회귀 R²를 재계산 → 어느 해부터 추세가 달라졌는지 탐지
  - 환자수 R²: 2010~ **0.7054** / 2016~ 0.7922 / **2017~ 0.9719** / 2018~ 0.9817 / 2021~ 0.9912
  - **채택: 2017년** — 이 시점은 **2017년 급여 적용 연령 15세→19세 확대**와 일치하여,
    통계적 근거와 제도적 근거가 상호 검증됩니다
  - ⚠️ **과적합(overfitting) 회피 규칙** — R²가 가장 높은 구간(2021~, 표본 5년)을 그냥 고르면
    표본이 적을수록 R²가 자동 상승하는 성질 때문에 예측이 과격해집니다. 따라서
    `R2_THRESHOLD = 0.95`를 만족하는 구간 중 **가장 이른(=표본 최대) 시작연도**를 채택합니다
    (`MIN_N = 6`). 결과: 2021년(n=5) → **2017년(n=9)**
- **연령대별 성장 기여도 분해** (`analytics.ageContribution`) — 5세 구간 원본을 5개 밴드로 재집계
  - 2017→2025 순증분(Δ) 중 각 연령대 몫: 0-19세 **-12.6%** / 20-39세 +6.6% / 40-59세 +25.0%
    / **60-74세 +53.1%** / 75+ +27.8% (합계 100%)
  - 구성비: 0-19세 46.1%→18.7%, 60-74세 17.0%→33.9%, 75+ 8.0%→17.3%
  - → 성장의 **절반 이상을 60-74세 단일 코호트가 만들었고**, 소아는 오히려 성장을 상쇄
  - 음수 기여율은 해당 코호트가 감소해 전체 성장을 깎았다는 의미입니다
- **환자당 수술건수** (`analytics.procPerPatient`) — 양측이식·재수술 **대리지표(proxy)**
  - 1.0441(2010) → 1.2159(2020, 최고) → 1.1411(2025). 2025년 환자수 초과 건수 **168건**
  - ⚠️ **해석 주의** — HIRA 공개통계에는 양측/재수술 구분 항목이 **없습니다**. 이 비율은
    양측이식·재수술이 늘면 함께 오르는 **간접 대리지표**이며, **양측이식률 자체가 아닙니다**
- **기관 종별 집중도** (`analytics.instConcentration`)
  - 상급종합 **환자** 비중 89.2% → 83.5% / **금액** 비중 90.7% → **89.0%**
  - 금액 비중 > 환자 비중 → **고액 케이스가 상급종합에 더 쏠림**
  - 기관 HHI 8,006 → **7,123** (2,500 초과 = 극단적 독점 구간 유지)
  - → 수요의 약 89%가 상급종합 채널. 소수 대형병원 채택 여부가 매출을 좌우하는 구조
- **성별 구성 추이** (`analytics.genderTrend`)
  - 여성 비중 전 기간 **45.6~52.9%** 범위에서 안정 → **성별은 유의미한 세분화 축이 아님**을 실측으로 확인
  - (유의미하지 않다는 결론도 실측으로 확인해 기록해 둡니다)

**🟠 추정 구역 — 시장 전망 (`forecast`, `isEstimate: true`)**

기준구간 **2017~2025년(9개 표본)** 실측 추세를 2026~2030년으로 외삽합니다.
**3개 방법을 하나로 합치지 않고 병렬 제시** — 방법 간 차이 폭 자체가 불확실성의 크기입니다.

| 방법 | 내용 |
|---|---|
| ① 선형회귀(OLS) | 매년 일정량 증가 가정 (보수적) |
| ② 로그선형 | 복리 성장 가정 (`exp(slope) - 1` = 연성장률) |
| ③ 기준구간 CAGR | 시작~끝 두 점만 사용한 연평균 성장률 |
| 예측 밴드 | `1.96 × 잔차표준오차 × √(1 + i/n)` — 3방법 min/max에서 ± 확장 |

| 지표 | 선형 R² | 로그 R² | 연성장 | 2026 (평균) | 2030 (평균) | 2030 범위 |
|---|---|---|---|---|---|---|
| 환자수 | 0.9719 | 0.9866 | 8.51% | 1,263명 | **1,680명** | 1,421 ~ 1,857 |
| 수술건수 | 0.9729 | 0.9824 | 7.73% | 1,440건 | **1,876건** | 1,615 ~ 2,053 |
| 진료금액 | 0.9401 | 0.9759 | 13.18% | 21.4억원 | **33.2억원** | 23.0 ~ 41.4억원 |

- ⚠️ **한계** — HIRA 실측 추세의 **외삽**이며, 급여기준 변경·신규 진입·수가 조정 등
  **정책 이벤트는 반영되지 않았습니다**. 실적이 아닌 참고용 추정치입니다
- 진료금액 R²(0.9401)가 환자수·건수보다 낮음 → **금액 전망의 불확실성이 가장 큽니다**

**🔴 추정 구역 — 기기(디바이스) 시장규모 (`deviceMarket`, `isEstimate: true`)**

산식: **수술건수(HIRA 실측) × 세트단가(공개 보도 기준 가정)**

| 시나리오 | 세트 단가 가정 | 2025년 (실측 1,359건) | 2030년 (추정 1,876건) |
|---|---|---|---|
| 보수적 | 1,500만원 | 204억원 | 281억원 |
| **기준** | **2,000만원** | **272억원** | **375억원** |
| 적극적 | 2,500만원 | 340억원 | 469억원 |

- ⚠️ **HIRA 진료금액과 혼동 금지** — HIRA 진료금액(요양급여비용)과는 **산식이 다른 별개 추정**입니다
- ⚠️ 세트단가는 **확인된 실거래가가 아닙니다**. 외부장치 단독 교체·부속품 매출이 제외되어
  **과소 추정**될 수 있습니다
- ⚠️ **불확실성 이중 누적** — 전망 연도 값은 「추정 건수 × 가정 단가」이므로 오차가 두 번 겹칩니다.
  인용 시 반드시 **단가 가정과 건수 추정 방법을 함께 명시**하세요

**분석에서 확인된 데이터 특성 (참고)**
- 인공와우이식술은 **사실상 100% 입원 시술** — 16개년 중 외래는 2015년 1건뿐입니다
- HIRA 원본에는 `gender='계'` × 입원/외래 조합이 없어, 입원/외래 집계는 **남+여 합산**으로 산출합니다
- 상급종합병원 비중 83.5% (2025) — 16개년 내내 78~89% 범위로 안정적
- 환자수 감소 연도: 2012 · 2015 · 2017 (3개년) / 평균 YoY +6.2%, 표준편차 12.8%p
- **원본 데이터 출처 링크 내장** — 화면 상단 「원본 데이터 보기」 버튼과 하단 출처 표기에서
  HIRA 원본 통계 페이지를 새 탭으로 엽니다 (공공누리 제1유형 출처표시 의무 이행)
  - 직접 진입: https://opendata.hira.or.kr/op/opc/olapDiagBhvInfoTab1.do
...
| 외부기 | 상태 · 모델명 · 시리얼 번호 · **이니셜** · **보안키** · 지급/교체일 · 버전 · 메모 |

### 외부기 방향(side) 제거

자사 외부기는 **좌우 구별 없이 착용**할 수 있어 방향이 실무에서 쓰이지 않았습니다.
(제거 시점 프로덕션 7건 **전부 `left`** — 폼 기본값이 그대로 저장된 것뿐이었습니다.)

- 등록/편집 폼의 **「방향 \*」 드롭다운 제거**, 목록의 **좌/우 배지 제거**
- **내부기는 그대로 좌/우 슬롯 유지** — 수술 부위와 직결되는 값입니다
- **`side` 컬럼은 삭제하지 않았습니다.** `NOT NULL CHECK (side IN ('left','right'))` 이라
  SQLite 에서 컬럼을 지우려면 테이블 재생성이 필요해 위험 대비 이득이 없었습니다.
  - `POST` : `side` 미지정이면 **`'left'` 로 자동 저장** (기존의 400 에러 응답 제거)
  - `PUT` : `side` 미전송이면 **기존 값을 그대로 유지** (`normSide(b.side) || own.side || 'left'`)
    → 과거에 `right` 로 저장된 건이 있어도 편집 저장 시 덮어써지지 않습니다 (로컬 검증 ✅)
- 활동 로그 문구도 `홍길동 · 좌측 외부기` → **`홍길동 · 외부기 (이니셜)`** 로 변경
- **방향을 다시 쓰려면** `app.js` 의 `_custEditExternal()` 에 드롭다운 한 블록만 되살리면 됩니다.
  백엔드는 `side` 가 오면 그대로 받도록 이미 열려 있습니다.
...
## 영업 전략 분석 — 어느 병원을 공략할 것인가 (2026-09-03)

인공와우 심층분석 탭 하단에 **HIRA 지역 시장 실측 × CRM 병원 실측 교차** 블록을 추가했습니다.
API: `GET /api/ci-stats` → `data.salesStrategy` (실측 전용, 추정 아님)

### 🔴 전제 — 반드시 알아야 할 데이터 한계
- **HIRA 공개통계에는 요양기관(병원)별 데이터가 없습니다.** 지역·기관종별까지만 공개되므로
  "어느 병원을 공략할지"는 HIRA 단독으로 도출 불가 → CRM 등록 병원 48곳과 교차했습니다.
- `hospitals.patient_count` / `ci_referrals` / `hearing_aid_sales` 는 **48행 전부 0(미입력)** 이라
  근거로 쓰지 않았습니다. 실제 기록이 있는 **미팅 횟수 · 파이프라인 단계 · 키맨 수**만 사용합니다.
  → 이 세 컬럼을 채우면 「같은 서울 안에서 어느 병원이 더 큰가」를 구분할 수 있어 정확도가 올라갑니다.

### 응답 필드
| 필드 | 내용 |
|---|---|
| `effortGap` | 지역별 시장 점유(환자%) vs 노력 점유(미팅%) · 배율 · 성장률 · 병원수 · 계약수 |
| `funnel` | 6단계(contact→active_customer) 병원 분포 + 직전단계 대비 전환율 |
| `stalled` | 마지막 미팅 60일 이상 경과 & 계약 전 = **19곳** |
| `neverMet` | 미팅 기록이 아예 없는 곳 = **6곳** (경과일 산출 불가 → 별도 분류) |
| `targets` | 공략 우선순위 점수(0~100) 순 **41곳** (계약·활성 제외) + 6축 내역 `parts` |
| `summary` | 커버리지 · 퍼널 요약 · 방치 건수 |

### 주요 결론
1. **「미개척 지역 발굴」은 답이 아님** — 병원 등록 지역의 시장 커버 **99.9%**(미커버는 세종 1명뿐),
   반면 계약 확보 지역 커버는 **64.2%**. 격차 35.7%p. 문제는 «커버»가 아니라 «전환»입니다.
2. **노력이 시장과 역배분** — 인천 **23.21배** 과잉(시장 연 −2.3% 축소) vs
   경기 0.71배(**+15.3%**, 계약 0) · 광주 0.45배(**+22.5%**, 계약 0).
   거점 공백(환자>0 & 계약 0) = 경기 · 광주 · 대구 · 강원 · 전북 · 울산 · 세종
3. **퍼널 «허리» 붕괴** — 미팅 21곳 → 데모 1곳 = 전환율 **5%**. 초기단계 39곳(81%), 중간단계 2곳.
4. **방치 파이프라인 25곳** — 60일 이상 19곳 + 미팅 기록 전무 6곳. 120일 이상 6곳.

### ⚠️ 계산 규칙 (수정 시 반드시 유지)
- **성장률 기준연도는 `breakYear`(2017)로 통일** — `const regBase = breakYear as number`.
  2010년 기준으로 계산하면 2017년 급여 확대(15→19세) 이전 등락이 섞여 왜곡됩니다.
  (광주 9.58% → **22.50%**, 경기 9.92% → **15.29%**, 경남 +8.71% → **−10.50%** 로 순위가 뒤집힙니다)
- **우선순위 6축 배점** — 시장규모 18(√압축) / 성장성 12 / 거점공백 15 / 과소투자 10 / 진행단계 25 / 관계자산 20.
  시장규모를 비중 그대로 쓰면 서울(56.5%)이 점수를 지배해 **상위권이 전부 서울**이 됩니다. 반드시 √로 압축하세요.
- **시장이 0인 지역은 배율을 `null`** 로 둡니다(무한대 방지). UI의 「가장 덜 방문한 지역」은
  `hospitals > 0` 조건을 반드시 포함하세요 — 없으면 병원 0곳인 세종(0배)이 1위로 잡힙니다.
- **퍼널 표는 stock(현재 재고) 분포**이므로 전환율이 100%를 넘을 수 있습니다(flow 아님). UI에 주의문 표기됨.
- `WON = ['contract','active_customer']` — `dashboard.ts:442` 의 「확보」 정의와 동일하게 유지하세요.

### 관련 파일
- `src/routes/cistats.ts` — `salesStrategy` 블록
- `public/static/app.js` — `renderCIContent` insight 분기 UI 6섹션 + 액션플랜 4단계,
  `renderCIChartsForTab` 의 `chart-effort-gap` / `chart-funnel` / `chart-priority`
- ⚠️ `/api/ci-stats/cross-analysis` 는 `WHERE status='active'` 필터로 **5곳만** 집계합니다.
  `salesStrategy`(48곳 전체)와 수치가 다른 것은 정상이며, 혼용하지 마세요.

## Deployment
- **Platform**: Cloudflare Pages + D1 Database
- **Status**: ✅ Production Active
- **Deployment URL**: https://todoc-crm.pages.dev
- **Last Updated**: 2026-09-03 (**영업 전략 분석 신설** — HIRA 지역 시장 실측 × CRM 병원 48곳 실측 교차로 「어느 병원을 공략할지 / 앞으로 어떻게 할지」 계량화 · 「미개척 지역 발굴」 결론 폐기(병원 등록 지역 시장 커버 99.9% vs 계약 확보 지역 커버 64.2%, 격차 35.7%p — 문제는 커버가 아니라 전환) · 노력-시장 역배분 규명(인천 23.21배 과잉인데 시장은 연 -2.3% 축소 vs 경기 0.71배·+15.3%·계약 0 / 광주 0.45배·+22.5%·계약 0) · 퍼널 허리 붕괴 규명(미팅 21곳 → 데모 1곳 = 전환율 5%, 초기단계 81%, 중간단계 2곳) · 방치 파이프라인 25곳(60일 이상 정체 19곳 + 미팅 기록 전무 6곳을 경과일 산출 불가로 별도 분류, 120일 이상 6곳) · 공략 우선순위 41곳 6축 채점(시장규모 18 제곱근압축/성장성 12/거점공백 15/과소투자 10/진행단계 25/관계자산 20 — 시장규모를 비중 그대로 쓰면 상위권이 전부 서울이 되어 압축, 결과 TOP10 = 경기 7·서울 3) · **성장률 기준연도를 구조 전환점 breakYear(2017)로 통일**(2010년 기준은 급여 확대 이전 등락이 섞여 왜곡 — 광주 9.58%→22.50%, 경기 9.92%→15.29%, 경남 +8.71%→-10.50% 로 순위 역전) · 액션 플랜 4단계 · 차트 3종 신설 · 로컬 D1에 프로덕션 스냅샷을 시딩해 서버 계산 실검증(파이썬 대조 불일치 0건), 프로덕션 D1은 읽기만 수행, 이전: **HIRA 심층 분석 확장 + 시장 전망 추정 추가** — 성장 국면 전환점 탐지로 **2017년**을 구조 변화 시점으로 규명(R² 0.7054→0.9719, 2017년 급여 연령 15→19세 확대와 시점 일치) · 과적합 회피 규칙(R²≥0.95 중 표본 최대 구간 채택)으로 기준구간 2021년(n=5)→2017년(n=9) 교정 · 연령대별 성장 기여도 분해(60-74세가 순증분의 53.1% 주도, 0-19세 -12.6%로 성장 상쇄) · 환자당 수술건수 대리지표(2025년 초과 168건, 양측이식률 아님 명기) · 기관 종별 집중도(HHI 8,006→7,123, 금액비중 89.0% > 환자비중 83.5%) · 성별 추이(45.6~52.9% 안정 = 유의미한 축 아님 확인) · **2026~2030 전망 3방법 병렬**(선형회귀/로그선형/CAGR + 예측밴드 — 2030년 환자 1,680명[1,421~1,857]·건수 1,876건[1,615~2,053]·금액 33.2억[23.0~41.4억]) · **기기 시장규모 3시나리오 추정**(1,500/2,000/2,500만원 → 2025년 272억원, 2030년 375억원 기준) · 실측/추정을 점선 테두리·배지·API 키(`analytics` vs `forecast`/`deviceMarket`)로 완전 분리 · HIRA 원본 데이터 링크를 `olapDiagBhvInfoTab1.do`로 교체, 이전: **HIRA 16개년 심층 분석 탭 신설** — 연령 코호트 역전(소아 56.1%→18.7% / 60세+ 14.5%→51.1%) · 지역 집중도 HHI · 진료금액 물량/단가 분해 · 지역별 CAGR, `app.js` 잔존 `6년간` 하드코딩 제거, 인공와우 통계를 2010~2025년 16개년으로 확대 · 1,424행 적재, 인사이트 연도 하드코딩 제거, 근거 미확인 2025년 정책 문구 삭제, HIRA 원본 데이터 출처 링크 추가, 외부기 방향(좌/우) 입력 제거, 보정 경유지가 있을 때 업체명·지역 어긋남 수정)
