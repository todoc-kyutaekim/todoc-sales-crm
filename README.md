...
##### 심층 분석 탭 (`심층 분석`)
16개년 데이터에서 계산한 구조 분석입니다. **모든 값은 HIRA 실측치 기반이며 추정·예측치를 섞지 않습니다.**
...
- **Last Updated**: 2026-09-03 (**HIRA 16개년 심층 분석 탭 신설** — 연령 코호트 역전(소아 56.1%→18.7% / 60세+ 14.5%→51.1%) · 지역 집중도 HHI · 진료금액 물량/단가 분해 · 지역별 CAGR, `app.js` 잔존 `6년간` 하드코딩 제거, 인공와우 통계를 2010~2025년 16개년으로 확대 · 1,424행 적재, 인사이트 연도 하드코딩 제거, 근거 미확인 2025년 정책 문구 삭제, HIRA 원본 데이터 출처 링크 추가, 외부기 방향(좌/우) 입력 제거, 보정 경유지가 있을 때 업체명·지역 어긋남 수정)
